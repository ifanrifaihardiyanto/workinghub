...
class Manageprofile_model extends CI_Model
{
    ...
    {
        $check = "select * from user left outer join $role on user.id = $role.id_user where user.id='$user_id'";
        if (empty($this->db->query($check)->row())) {
            $sql = "insert into $role (name, place_birth, date_birth, address, nik, no_tlp) values ('$nama','$tmptLahir','$tglLahir','$alamat','$nik','$noTelp')";
        } else {
            if ($role === 'admin') {
